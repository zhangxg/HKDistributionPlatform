...
Ext.define('Ext.chooser.IconBrowser', {
        ...
        // '<div class="details">',
            '<tpl for=".">',
                '<div class="thumb-wrap">',
                    '<div class="thumb">',
                    (!Ext.isIE6? '<img src="../resources/img/product/{thumb}" />' : 
                    '<div style="width:74px;height:74px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'../resources/img/product/{thumb}\')"></div>'),
                    '</div>',
                    '<span>{name}</span>',
                '</div>',
            '</tpl>'
    ...
    initComponent: function() {
        this.store = Ext.create('Ext.data.Store', {
            ...
            proxy: {
                type: 'ajax',
                url : '../resources/js/sitepro/icons.json',
                reader: {
                    type: 'json',
                    root: ''
                }
